...
imessage_url = 'iMessage://79132056035'  # URL to iMessage sender (sorry, but we did some RE for that :) )
iwdev = 'wlan0'
...
exist_devs = []
def write_results_csv(row):
    global exist_devs
    with open('tes.csv', newline='') as File:
        reader = csv.reader(File, delimiter=' ', quotechar=';')
        for line in reader:
            if line:
                exist_devs.append(line[0])

    with open('tes.csv', 'a', newline='') as File:
        fieldnames = ['MAC', 'STATE', 'DEVICE', 'WI-FI','OS', 'PHONE', 'TIME', 'NOTES']
        writer = csv.DictWriter(File, fieldnames=fieldnames)
        for dev in range(len(row)):
            if row[dev][0] not in exist_devs:
                writer.writerow({'MAC': row[dev][0], 'STATE': row[dev][1], 'DEVICE': row[dev][2], 'WI-FI': row[dev][3], 'OS': row[dev][4], 'PHONE': row[dev][5], 'TIME': row[dev][6], 'NOTES': row[dev][7]})
                exist_devs.append(row[dev][0])

def test_results(info):
    f = open('test.txt', 'a')
    #for line in info:
    if info:
        f.write(str(info))
...
def print_results():
    ...
    for phone in phones:
        mac = phone
        device_name = get_dev_name(mac)
        if not device_name:
            device_name = phones[phone]['device']
        row.append([phone, phones[phone]['state'], device_name, phones[phone]['wifi'], phones[phone]['os'],
                    phones[phone]['phone'], phones[phone]['time'], phones[phone]['notes']])
        
    write_results_csv(row)
    return row
